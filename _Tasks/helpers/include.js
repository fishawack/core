module.exports = function(grunt, hasBase, fixture) {
	...
	this.captureEnv = function(){
		return {
	        ...
	        pages: contentJson.attributes.capture && contentJson.attributes.capture.pages || require('glob').sync(`**/*.html`, {cwd: contentJson.attributes.root || '_Output'}).map(d => `/${d}`).sort(),
...
module.exports.deployBranch = () => {
	try{
		return process.env.BRANCH || process.env.CI_COMMIT_REF_NAME || require('child_process').execSync(`git branch --show-current`, {encoding: 'utf8'}).trim();
   } catch(e){
	   return 'unknown';
   }
}
